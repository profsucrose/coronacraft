...
public class ToggleChannel implements CommandExecutor {
    ...
    private static void fillArea(World world, int x, int y, int z, Material block) {
        for (int mapY = 0; mapY < 50; mapY++) {
            for (int mapX = 49; mapX > -1; mapX--)
                world.getBlockAt(x + mapX, y, z + mapY).setType(block);
        }
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (isBlocked) {
            fillArea(player.getWorld(), x, 5, y, Material.BLACK_CONCRETE);
        } else {
            fillArea(player.getWorld(), x, 5, y, Material.AIR);
        }
